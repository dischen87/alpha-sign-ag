#!/usr/bin/env node
/**
 * Migration Script: Import existing projects from TypeScript to Sanity
 * Run with: node migrate-projects.mjs
 */
...
// Existing projects data (copied from src/data/projects.ts)
const projects = [
  {
...
// Transform project to Sanity document format
function transformProject(project) {
  return {
    ...
    slug: {
      _type: 'localizedSlug',
      de: { _type: 'slug', current: project.slug },
      en: { _type: 'slug', current: project.slug },
    },
    client: project.client,
    completionDate: `${project.year}-01-01`,
    excerpt: {
      _type: 'localizedText',
      de: project.de.description,
      en: project.en.description,
    },
    challenge: {
      _type: 'localizedText',
      de: project.de.challenge,
      en: project.en.challenge,
    },
    solution: {
      _type: 'localizedText',
    ...
    // Note: Results are stored as localizedText for now (formatted as list)
    results: {
      _type: 'localizedText',
      de: project.de.results.map(r => `• ${r}`).join('\n'),
      en: project.en.results.map(r => `• ${r}`).join('\n'),
    },
    featured: true,
    // Note: Images need to be uploaded separately through Sanity Studio
    // For now, we store the external URLs in a custom field
    _externalMainImage: project.image,
    _externalGallery: project.gallery,
    _categoryKey: project.categoryKey,
  };
}
...
async function migrateProjects() {
  ...
  if (!process.env.SANITY_API_TOKEN) {
    console.log('⚠️  No SANITY_API_TOKEN found.');
    console.log('');
    console.log('To run this migration, you need to:');
    console.log('1. Go to https://www.sanity.io/manage/project/p8qni8zq');
    console.log('2. Navigate to API → Tokens');
    console.log('3. Create a new token with "Editor" permissions');
    console.log('4. Run: SANITY_API_TOKEN=your-token node migrate-projects.mjs');
    console.log('');
    console.log('Alternatively, you can manually create the projects in Sanity Studio:');
    console.log('👉 https://alpha-sign.sanity.studio/');
    console.log('');
    
    // Output JSON for manual import
    console.log('📋 Here is the data in NDJSON format for manual import:');
    console.log('---');
  ...
  for (const project of projects) {
    const doc = transformProject(project);
    console.log(`📝 Adding: ${doc.title.de}`);
  ...
  try {
    ...
    console.log('\n👉 Open Sanity Studio to review: https://alpha-sign.sanity.studio/');
    console.log('\n⚠️  Note: Images were not imported. Please upload them manually in Sanity Studio.');
  } catch (error) {
